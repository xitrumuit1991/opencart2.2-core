<?php

// Heading

$_['heading_title']          = 'Sản phẩm'; 



// Text  

$_['text_success']      = 'Thành công: Sản phẩm đã được thay đổi!';

$_['text_list']              = 'Danh sách sản phẩm';

$_['text_add']               = 'Thêm sản phẩm';

$_['text_edit']              = 'Chỉnh sửa sản phẩm';

$_['text_plus']              = '+';

$_['text_minus']             = '-';

$_['text_default']           = 'Mặc định';

$_['text_image_manager']     = 'Quản lý ảnh';

$_['text_browse']            = 'Duyệt file';

$_['text_clear']             = 'Xóa Ảnh';

$_['text_option']            = 'Tùy chọn';

$_['text_option_value']      = 'Giá trị tùy chọn';

$_['text_percent']           = 'Tỷ lệ phần trăm';

$_['text_amount']            = 'Số lượng Cố định';



// Column

$_['column_name']            = 'Tên sản phẩm';

$_['column_model']           = 'Model:';

$_['column_image']           = 'Hình ảnh';

$_['column_price']           = 'Giá';

$_['column_quantity']        = 'Số lượng';

$_['column_status']          = 'Trạng thái';

$_['column_action']          = 'Thao tác';



// Entry

$_['entry_name']             = 'Tên sản phẩm:';

$_['entry_meta_title'] 	     = 'Tiêu đề mở rộng';

$_['entry_meta_keyword']     = 'Từ khóa:';

$_['entry_meta_description'] = 'Mô tả từ khóa:';

$_['entry_description']      = 'Mô tả sản phẩm:';

$_['entry_store']            = 'Cửa hàng:';

$_['entry_keyword']          = 'Đường dẫn thân thiện SEO:';

$_['entry_model']            = 'Model:';
$_['entry_sku']              = 'SKU:';
$_['entry_bookname']         = 'Tên sách:';
$_['entry_author']           = 'Tác giả:';
$_['entry_translator']       = 'Dịch giả:';
$_['entry_isbn13']           = 'ISBN-13:';
$_['entry_binding']          = 'Trình bày Bìa sách:';
$_['entry_publishingdate']   = 'Ngày phát hành:';
$_['entry_publisher']        = 'Nhà xuất bản:';
$_['entry_numberofpages']    = 'Số trang:';
$_['entry_booklanguage']         = 'Ngôn ngữ:';
$_['entry_madein']         = 'Xuất xứ:';
$_['entry_material']         = 'Chất liệu:';
$_['entry_upc']              = 'UPC:';

$_['entry_ean']              = 'EAN: ';

$_['entry_jan']              = 'JAN: ';

$_['entry_isbn']             = 'ISBN: ';

$_['entry_mpn']              = 'MPN: ';

$_['entry_location']         = 'Địa điểm:';

$_['entry_manufacturer']     = 'Hãng sản xuất:';

$_['entry_shipping']         = 'Yêu cầu Vận chuyển:'; 

$_['entry_date_available']   = 'Ngày cập nhật:';

$_['entry_quantity']         = 'Số lượng:';

$_['entry_minimum']          = 'Số lượng tối thiểu: ';

$_['entry_stock_status']     = 'Hết hàng: ';

$_['entry_price']            = 'Giá:';

$_['entry_tax_class']        = 'Loại Thuế:';

$_['entry_points']           = 'Điểm: ';

$_['entry_option_points']    = 'Điểm:';

$_['entry_subtract']         = 'Phần Trừ đi:';

$_['entry_weight_class']     = 'Trọng lượng:';

$_['entry_weight']           = 'Nặng:';

$_['entry_store']            = 'Các gian hàng';

$_['entry_length']           = 'Chiều dài:';

$_['entry_length_class']     = 'Đươn vị chiều dài';

$_['entry_width']            = 'Chiều Rộng';

$_['entry_height']           = 'Chiều Cao';

$_['entry_dimension']        = 'Kích thước(Dài x Rộng x Cao):';

$_['entry_image']            = 'Hình ảnh:';

$_['entry_customer_group']   = 'Nhóm khách hàng:';

$_['entry_date_start']       = 'Từ ngày:';

$_['entry_date_end']         = 'Đến ngày:';

$_['entry_priority']         = 'Ưu tiên:';

$_['entry_attribute']        = 'Thuộc tính:';

$_['entry_attribute_group']  = 'Nhóm thuộc tính:';

$_['entry_text']             = 'Văn bản:';

$_['entry_option']           = 'Tùy chọn:';

$_['entry_option_value']     = 'Giá trị Tùy chọn:';

$_['entry_required']         = 'Bắt buộc:';

$_['entry_status']           = 'Trạng thái:';

$_['entry_sort_order']       = 'Thứ tự:';

$_['entry_category']         = 'Danh mục:';

$_['entry_filter']           = 'Bộ lọc: ';

$_['entry_download']         = 'Tải về:';

$_['entry_related']          = 'Sản phẩm cùng loại:';

$_['entry_tag']              = 'Tags:';

$_['entry_reward']           = 'Điểm thưởng:';

$_['entry_layout']           = 'Bố cục:';

$_['entry_profile']          = 'Hồ sơ sản phẩm:';



$_['text_recurring_help']    = 'Khoản được thanh toán theo chu kỳ và định kỳ. <br />Ví dụ: Tần suát là 2, chu kỳ là 1 tuần tức là sẽ lập hóa đơn 1 tuần 2 lần, Thời hạn thanh toán định kỳ là: số lần khách hàng phải thanh toán. Đặt là 0 nếu yêu cầu thanh toán đến lúc khách hàng hủy bỏ.';

$_['text_recurring_title']   = 'Các khoản thanh toán định kỳ';

$_['text_recurring_trial']   = 'Giai đoạn dùng thử';

$_['entry_recurring']        = 'Thanh toán định kỳ:';

$_['entry_recurring_price']  = 'Giá định kỳ:';

$_['entry_recurring_freq']   = 'Tần suất:';

$_['entry_recurring_cycle']  = 'Chu kỳ: ';

$_['entry_recurring_length'] = 'Thời hạn thanh toán định kỳ: ';

$_['entry_trial']            = 'Giai đoạn dùng thử:';

$_['entry_trial_price']      = 'Giá dùng thử:';

$_['entry_trial_freq']       = 'Tần suất dùng thử:';

$_['entry_trial_cycle']      = 'Chu kỳ dùng thử: ';

$_['entry_trial_length']     = 'Thời hạn thanh toán dùng thử:';



$_['text_length_day']        = 'Ngày';

$_['text_length_week']       = 'Tuần';

$_['text_length_month']      = 'Tháng';

$_['text_length_month_semi'] = 'Hàng tháng';

$_['text_length_year']       = 'Năm';

// Help

$_['help_keyword']           = 'Không để khoảng trống, bạn có thể dùng ký tự - để thay thế khoảng trắng.';

$_['help_sku']               = 'Mã phân loại hàng hóa tồn kho';
$_['help_bookname']         = 'Tên sách:';
$_['help_author']           = 'Tác giả:';
$_['help_translator']       = 'Dịch giả:';
$_['help_isbn13']           = 'Mã số tiêu chuẩn cho sách dạng 13 ký tự:';
$_['help_binding']          = 'Trình bày Bìa sách:';
$_['help_publishingdate']   = 'Ngày phát hành:';
$_['help_publisher']        = 'Nhà xuất bản:';
$_['help_numberofpages']    = 'Số trang:';
$_['help_booklanguage']         = 'Ngôn ngữ:';
$_['help_madein']         = 'Xuất xứ:';
$_['help_material']         = 'Chất liệu:';
$_['help_upc']               = 'Mã số hàng hóa toàn cầu UPC';

$_['help_ean']               = 'Mã số hàng hóa theo tiêu chuẩn Châu Âu';

$_['help_jan']               = 'Mã số hàng hóa theo tiêu chuẩn Nhật Bản';

$_['help_isbn']              = 'Mã số tiêu chuẩn quốc tế cho sách';

$_['help_mpn']               = 'Mã số sản phẩm (của nhà sản xuất)';

$_['help_manufacturer']      = '(Tự động)';

$_['help_minimum']           = 'Số lượng tối thiểu cho mỗi đơn hàng';

$_['help_stock_status']      = 'Trạng thái hiển thị trên giang hàng khi trong kho đã hết hàng';

$_['help_points']            = 'Số điểm tích lũy để mua sản phẩm này, nếu không yêu cầu, bạn hãy đặt giá trị 0.';

$_['help_category']          = '(Tự động)';

$_['help_filter']            = '(Tự động)';

$_['help_download']          = '(Tự động)';

$_['help_related']           = '(Tự động)';

$_['help_tag']          	 = 'Cách nhau bằng dấu phẩy(,)';



// Error

$_['error_warning']          = 'Cảnh báo: Dữ liệu bắt buộc chưa được nhập vào. Kiểm tra các ô trống!';

$_['error_permission']       = 'Cảnh báo: Bạn không được phép thay đổi các sản phẩm!';

$_['error_name']             = 'Tên sản phẩm phải lớn hơn 3 và nhỏ hơn 255 ký tự!';

$_['error_meta_title']       = 'Meta Title phải lớn hơn 3 và nhỏ hơn 255 ký tự!';

$_['error_model']            = 'Model sản phẩm phải lớn hơn 3 và nhỏ hơn 64 ký tự!';

$_['error_keyword']          = 'Liên kết này đã được dùng!';

